(
  ...
  if [ -d "$clash_dir" ]; then
    ...
    cp "$clash_dir/Clash配置.yaml" "$backup_dir/Clash配置.yaml" 2>/dev/null
    cp "$clash_dir/工具/自定义直连.yaml" "$backup_dir/自定义直连.yaml" 2>/dev/null
    cp "$clash_dir/工具/自定义代理.yaml" "$backup_dir/自定义代理.yaml" 2>/dev/null
